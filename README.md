...
![Версия прошивки TX](https://img.shields.io/badge/Версия_прошивки-TX_6.2-blue) 
...
![Протокол связи LoRa](https://img.shields.io/badge/Технология-LoRa-blueviolet)


![Тест связи 1](тест1.jpg)
![Тест связи 2](тест2.jpg)
![Сборка RX](Общий вид приемника.jpg)
![Сборка TX](Общий вид передатчика.jpg)
---
...
- Диапазон температуры от -40 до +50
...
LoRa:
- Рабочая частота 850-931 МГц
- Мощность 28dBm
- Рабочая температура -40°C ... +85°C
- Антенна усиление 5 дБи
...
5. **Онлайн монитор:**
   ...
   ![screen](screen/capture_260212_1240071.png)
...
- Единый пакет телеметрии:
```
ALL:TEMP:<t>|GPS:<lat>,<lng>|PZEM:<v>,<i>,<p>,<e>,<f>,<pf>|STAx
```
- Примечания:
  - `<t>` — температура или `--`, если датчик недоступен
  - `<lat>,<lng>` — координаты или `--`, если GPS недоступен
  - `STAx` — статус реле: `STA0` (выключено) или `STA1` (включено)
...
| `ACKA` | Подтверждение получения пакета `ALL` (включая частичные данные)
...
Примечания:
- Пакеты могут приходить фрагментами; контроллер связывает заголовок `[XX]` с последующим payload.
- Таймаут накопления полезной части: 400 мс.
...
- [ ] Контроль трехфазного напряжения
